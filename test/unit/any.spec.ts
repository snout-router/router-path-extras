...
describe("any()", () => {
  ...
  it("should allow building with alternate expressions, separators, and prefixes", () => {
    expect(
      path`/a${any("p1", /x/, "<sep>")}/b`.build({ p1: ["wx", "yz"] }),
    ).toBe("/a<sep>wx<sep>yz/b");
    expect(
      path`/a${any("p1", /x/, "<sep>", "<pre>")}/b`.build({ p1: ["wx", "yz"] }),
  ...
  it("should allow matching with alternate expressions, separators, and prefixes", () => {
    ...
    expect(subjectA.match("/a<sep>xy<sep>yz<sep>𝓍<sep>𝓎/b")).toStrictEqual({
      p1: ["xy", "yz", "𝓍", "𝓎"],
    });
    ...
    expect(subjectB.match("/a<pre>xy<sep>yz<sep>𝓍<sep>𝓎/b")).toStrictEqual({
      p1: ["xy", "yz", "𝓍", "𝓎"],
    });
  });
  ...
  it("should complain about keys being possibly undefined", () => {
    ...
    // @ts-expect-error should expect index 0 to be possibly undefined
